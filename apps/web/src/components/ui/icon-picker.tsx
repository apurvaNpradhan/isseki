...
import type { iconsData } from "@/constants/icons";
...
interface IconPickerProps
	...
	defaultIconValue?: IconName;
	onColorValueChange?: (value: string) => void;
	onIconValueChange?: (value: IconName) => void;
	open?: boolean;
	defaultOpen?: boolean;
...
const useIconsData = () => {
	...
	useEffect(() => {
		...
		const loadIcons = async () => {
			...
			const { iconsData } = await import("@/constants/icons");
			if (isMounted) {
				setIcons(iconsData);
				setIsLoading(false);
			}
	...
	(
		...
		const handleIconValueChange = useCallback(
			(icon: IconName) => {
				...
				onIconValueChange?.(icon);
			},
			[IconValue, onIconValueChange],
		);
		...
		const handleIconClick = useCallback(
			(iconName: IconName) => {
				handleIconValueChange(iconName);
		...
		return (
			...
			>
					...
					{children || (
						...
						>
							{IconValue || selectedIcon ? (
								<Icon
									name={(IconValue || selectedIcon)!}
									color={selectedColor}
									className="transition-colors duration-200 group-hover:brightness-85"
								/>
							) : (
								triggerPlaceholder
							)}
				...
				<PopoverContent className="flex w-64 flex-col p-2">
					<div className="mt-2 flex flex-row gap-1 overflow-x-auto pb-2">
						{COLORS.map((color) => (
							<button
								...
								key={color}
								className={cn(
									"rounded-md p-1 transition hover:bg-foreground/10",
									"flex items-center justify-center",
									color === selectedColor ? "bg-foreground/10" : "",
								)}
